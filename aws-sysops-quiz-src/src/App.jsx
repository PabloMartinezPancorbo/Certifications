...
networking: {
  ...
  sections: [
      ...
      content: [
        {
          ...
          table: {
            title: 'Comparison',
            headers: ["Feature", "DNS", "CNAME", "A Record", "ALIAS Record", "MX Record", "TXT Record"],
            rows: [
              ["Function", "The overall system for name resolution", "Creates an alias from one domain to another", "Points a domain name to an IP address (IPv4)", "AWS-specific record that points to AWS resources", "Specifies the mail servers for a domain", "Stores text strings or machine-readable data in DNS"],
              ["Purpose", "Translates domain names into IP addresses", "Redirects traffic or uses a nickname for a service", "Connects a domain directly to a server's IP address", "Maps a domain to AWS resources (ELB, CloudFront, S3) at the zone apex", "Directs email traffic to the appropriate mail exchange servers", "Used for domain verification, email authentication, and miscellaneous metadata"],
              ["Example", "www.google.com → 142.250.184.142", "www.example.com (alias) → example.com (CNAME) or blog.example.com (alias) → shop.example.com (CNAME)", "example.com → 192.0.2.1", "example.com → ALB (my-alb-123.us-east-1.elb.amazonaws.com)", "example.com MX 10 mail1.example.net, MX 20 mail2.example.net", "example.com TXT \"v=spf1 include:_spf.example.com ~all\""],
              ["Relationship to others", "Contains all record types (A, CNAME, MX, TXT, ALIAS, etc)", "Points to a canonical name, which is then resolved via an A/AAAA record", "Points to the final IP address that the browser or client will connect to", "Similar to CNAME but works at zone apex; Route 53 automatically resolves to IP addresses", "Works together with A/AAAA of the mail servers; must point to a hostname (not a CNAME) per DNS spec", "Often complements MX records or other records; clients read TXT for policies/verification"],
              ["Use case", "Any website or online service requiring name resolution", "When you want multiple names/aliases to point to a single canonical domain (cannot use at zone apex)", "When you have a stable IPv4 address and want direct control over it", "When pointing zone apex to AWS resources (ELB, CloudFront, S3 website, API Gateway); no charge for queries", "When you host email for the domain and need prioritized mail servers", "When you must configure domain ownership validation, email sender policy (SPF), DKIM, or other metadata"],
              ["Zone Apex Support", "N/A", "No - CNAME not allowed at zone apex", "Yes", "Yes - main advantage over CNAME", "Yes", "Yes"],
              ["AWS Specific", "No", "No", "No", "Yes - Route 53 only", "No", "No"]
            ]
          },
          resources: [
    ...
    {
      title: 'AWS Network Firewall & Route 53 Resolver DNS Firewall',
      content: [
        {
